...
//    MoveCreator::clusterRotateD(cluster_i, 90.0 , Vector(0,0,1)); // do rotation
    ...
    energy_new = calcEnergy->allToAllTrial();
    ...
    if (sim->pseudoRank % 2 == oddoreven) {
        ...
        if (sim->pseudoRank+1 < sim->mpinprocs ) { // all except MAX
            ...
            if (wl.wlm[0] > 0) {
                ...
                change += (-wl.shared_weights[localwl] + wl.shared_weights[receivedwl] )/sim->temper + ( -recwlweights[receivedwl] + recwlweights[localwl])/receivedmpi.temperature ;
            }
